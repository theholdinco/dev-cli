...
_dev() {
  ...
  commands="init setup new ls attach kill hub dashboard ports logs url supabase pr shell worktree img projects update help status restart send diff sync gc rename template config agent stats doctor web bot services notify mobile m task ask ghost review vibes fortune lenny"

  # Commands that take a session name as argument
  session_cmds="attach kill logs url shell pr status restart send diff sync rename review"
