...
_dev() {
  ...
  local commands="setup new ls attach kill hub dashboard ports logs url supabase pr pr-start shell worktree img projects update help status restart yolo send diff sync gc rename private template config agent claude codex stats doctor web bot services notify mobile m task ask ghost scaffold review vibes fortune lenny linear admin setup-account"

  # Commands that take a session name as second argument
  local session_cmds="attach kill logs url supabase shell pr status restart yolo send diff sync rename private review claude codex"
